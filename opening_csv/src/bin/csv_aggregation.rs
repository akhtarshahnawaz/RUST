...
impl DataSet {
    ...
    /// Note: order to make Vect of Struct sortable you need four traits Eq, PartialEq, Ord and PartialOrd.
    /// So csv crate must have implemented these traits for StringRecord
...
impl Aggregate for DataSet {
    ...
    /// Notice that 'self' refers to specific instance of object that he method is called on
    /// Whereas 'Self' refers to datatype (Dataset in this case) of object on which it is called
    fn inner_join(&mut self, right: &mut Self, key: &str) -> Result<DataSet, Box<dyn Error>> {
        // Get column index
        ...
        while left_cursor < self.records.len() && right_cursor < right.records.len() {
            // If two fields match, merge fields into a single record and add to records vector
            // If they don't match and the left value is less then right value advance the left cursor else advance the right cursor
            if self.records[left_cursor][left_index] == right.records[right_cursor][right_index] {
                ...
                // Since data sets are sorted, Advance cursor through right data set to see if there are matches
